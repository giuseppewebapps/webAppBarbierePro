...
import { collection, query, where, orderBy, onSnapshot, doc, updateDoc, Timestamp, getDoc, getDocs, addDoc } from 'firebase/firestore';
import { db } from '../firebase';
import { Appointment, UserProfile, RescheduleProposal, TimeRange, SpecialDay, StaffProfile } from '../types';
...
export default function MultiBarberDashboard({ selectedAppointmentId, selectedNotificationType, onAppointmentDialogClose }: Props) {
  ...
  const [activeMobileTab, setActiveMobileTab] = useState<string | 'all'>('all');
  ...
  const [showGapFiller, setShowGapFiller] = useState<{ start: Date, end: Date, appointmentId?: string, staffId?: string | null } | null>(null);
  ...
  useEffect(() => {
    if (viewMode !== 'daily' && activeMobileTab === 'all' && staffMembers.length > 0) {
      setActiveMobileTab(staffMembers[0].uid);
    }
  }, [viewMode, activeMobileTab, staffMembers]);
  ...
  useEffect(() => {
    if (!tenantId) return;
    const unsubscribe = onSnapshot(query(collection(db, 'salons', tenantId, 'staff'), where('active', '==', true)), (snapshot) => {
      setStaffMembers(snapshot.docs.map(d => d.data() as StaffProfile).sort((a, b) => a.order - b.order));
    });
    return () => unsubscribe();
  }, [tenantId]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      let ownerId = staffMembers.find(s => s.role === 'owner')?.uid;
      if (!ownerId && staffMembers.length > 0) ownerId = staffMembers[0].uid;
  ...
  const isMatchedBySearch = (app: Appointment & { customer?: UserProfile }) => {
    ...
    if (searchType === 'name') {
      return getDisplayName(app).toLowerCase().includes(searchLower);
    } else {
  ...
  const findCandidatesForGap = (clickedGap: { start: Date, end: Date, appointmentId?: string, staffId?: string | null }) => {
    if (!salonSettings?.weeklySchedule) return;
    const dayApps = appointments.filter(a => isSameDay(a.startTime.toDate(), clickedGap.start) && a.status === 'booked' && (!clickedGap.staffId || a.staffId === clickedGap.staffId)).sort((a, b) => a.startTime.toMillis() - b.startTime.toMillis());
  ...
  const calendarItems = viewMode === 'daily' 
    ...
    : eachDayOfInterval({ 
        start: startOfWeek(selectedDate, { weekStartsOn: 1 }), 
          ...
          : endOfDay(addDays(startOfDay(new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 0)), 0)) 
      });

  const columnsToRender = viewMode === 'daily' 
    ? (activeMobileTab === 'all' ? staffMembers : staffMembers.filter(s => s.uid === activeMobileTab)) 
    : staffMembers.filter(s => s.uid === activeMobileTab);

  if (loading) return <div className="text-center py-12">Caricamento dashboard Kanban...</div>;

  return (
    <div className="space-y-6 max-w-[1400px] mx-auto pb-12 px-2 sm:px-4">
      <div className="bg-white/90 backdrop-blur-md border border-white/20 rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div className="p-4 sm:p-6 border-b border-gray-100/50 flex flex-col gap-4">
          
          {/* Riga 1: Navigazione Data e Toggle Vista */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
             <div className="flex items-center justify-between w-full sm:w-auto gap-2">
              ...
              <div className="flex items-center justify-center gap-1">
                <h3 className="text-lg sm:text-xl font-bold text-center capitalize min-w-[160px]">
                  {viewMode === 'monthly' ? format(selectedDate, 'MMMM yyyy', { locale: it }) : format(selectedDate, 'EEEE d MMM', { locale: it })}
                </h3>
                <div className="relative w-10 h-10 flex items-center justify-center cursor-pointer overflow-hidden rounded-full">
            ...
            {/* Toggle Giorno/Settimana/Mese Centrale */}
            <div className="flex justify-center w-full sm:w-auto">
              <div className="flex items-center bg-gray-100 rounded-full p-1 shadow-inner">
                <button onClick={() => setViewMode('daily')} className={cn("px-4 py-1.5 rounded-full text-xs font-bold transition-all", viewMode === 'daily' ? "bg-black text-white shadow-md" : "text-gray-600 hover:text-black")}>Giorno</button>
                <button onClick={() => setViewMode('weekly')} className={cn("px-4 py-1.5 rounded-full text-xs font-bold transition-all", viewMode === 'weekly' ? "bg-black text-white shadow-md" : "text-gray-600 hover:text-black")}>Settimana</button>
                <button onClick={() => setViewMode('monthly')} className={cn("px-4 py-1.5 rounded-full text-xs font-bold transition-all", viewMode === 'monthly' ? "bg-black text-white shadow-md" : "text-gray-600 hover:text-black")}>Mese</button>
              </div>
            </div>
          </div>

          {/* Riga 2: Ricerca e Azioni Veloci */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
             <div className="relative flex w-full sm:max-w-md shadow-sm rounded-xl">
              <select value={searchType} onChange={(e) => setSearchType(e.target.value as 'name' | 'phone')} className="bg-gray-50 border-y border-l border-gray-200 text-xs sm:text-sm rounded-l-xl px-2 py-2 outline-none focus:border-black font-medium text-gray-600 cursor-pointer shrink-0">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-center">
              <button onClick={() => setSelectedDate(new Date())} className="text-xs font-bold text-gray-400 hover:text-black shrink-0">Oggi</button>
              <button onClick={() => setIsReminderModalOpen(true)} className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 bg-[#25D366]/10 text-[#25D366] text-xs font-bold rounded-xl hover:bg-[#25D366]/20 transition-all shadow-sm shrink-0">
                <MessageCircle size={16} /> 
                <span className="inline">Invio Promemoria</span>
              </button>
            </div>
          </div>
        </div>

        {/* 🚀 TAB BARBIERI APPARE SEMPRE (Così si può navigare nella vista settimana) */}
        <div className="flex overflow-x-auto border-b border-gray-100 bg-gray-50/50 p-2 gap-2 scrollbar-hide">
          {viewMode === 'daily' && (
            <button onClick={() => setActiveMobileTab('all')} className={cn("px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap border transition-all", activeMobileTab === 'all' ? "bg-black text-white border-black shadow-md" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300")}>
              Tutti i Barbieri
            </button>
          )}
          {staffMembers.map(staff => (
            <button key={staff.uid} onClick={() => setActiveMobileTab(staff.uid)} className={cn("px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 whitespace-nowrap border transition-all", activeMobileTab === staff.uid ? "bg-white border-black ring-1 ring-black text-black shadow-sm" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300")}>
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: staff.color }} /> {staff.displayName.split(' ')[0]}
            </button>
          ))}
        </div>
        ...
        {/* 🚀 GRIGLIA KANBAN COMPLETA CON VISTE */}
        <div className="flex divide-x divide-gray-100 overflow-x-auto min-h-[600px]">
          {/* Sezione di Sinistra: Colonne Tempi o Giorni */}
                ...
                <div key={staffObj?.uid || 'mono'} className="flex-1 flex flex-col min-w-[300px] max-w-full">
                  ...
                  <div className="flex flex-col flex-1">
                    {calendarItems.map(item => {
                      ...
                      // 🚀 RENDERIZZAZIONE VISTA SETTIMANA E MESE
                      if (viewMode !== 'daily') {
                        const dayApps = filteredAppointments.filter(a => {
                          if (a.status === 'cancelled') return false;
                          if (staffObj && a.staffId !== staffObj.uid) return false;
                          return isSameDay(a.startTime.toDate(), item);
                        }).sort((a,b) => a.startTime.toMillis() - b.startTime.toMillis());

                        return (
                          <div key={item.toISOString()} className="min-h-[68px] border-b border-gray-50/50 p-2 flex gap-3 items-center overflow-x-auto scrollbar-hide relative">
                            <div className="flex gap-2 items-center flex-1">
                              {dayApps.length === 0 ? (
                                <div className="text-center text-gray-300 text-[10px] font-bold uppercase tracking-widest italic opacity-50 w-full pt-2">Libero</div>
                              ) : (
                                dayApps.map(app => {
                                  ...
                                  return (
                                    <div 
                                      ...
                                      className="shrink-0 p-2.5 rounded-xl shadow-sm flex flex-col justify-between cursor-pointer border border-black/5 bg-black text-white min-w-[160px] sm:min-w-[200px] max-w-[70vw] hover:scale-[1.02] transition-transform"
                                    >
                      ...
                      const overlappingApps = filteredAppointments.filter(a => {
                        if (a.status === 'cancelled') return false;
                        if (salonSettings?.hasMultiStaff && staffObj && a.staffId !== staffObj.uid) return false;
